...
// Завдання 5
/**
 * Функція `decodeURIComponentWrapper` виконує декодування рядка `encodedString` з використанням функції `decodeURIComponent`.
 * Якщо сталася помилка URIError, вона перехоплюється та виводиться повідомлення про помилку.
...
function decodeURIComponentWrapper(encodedString) {
	...
	// Якщо виникла помилка, і ії назва дорівнює URIError повертаємо помилку про неправильний URI формат з повідомленням Помилка декодування URI, інкше повертаємо текстове представлення помилки
	try {
...
function findEvenNumber(arr) {
	...
	try {
		evenNumbers = arr.find((number) => number % 2 === 0);
...
function processData(data) {
	// Для кожного елемента в масиві перевіряємо, чи елемент є числом
 ...
 * Використовується функція `eval` для обчислення виразу. Якщо виникає помилка EvalError, вона перехоплюється та виводиться повідомлення про помилку.
...
function evaluateExpression(expression) {
	...
	try {
		// Використовуємо функцію eval для обчислення виразу
		return eval(expression);

	} catch (error) {

		// Якщо виникає помилка EvalError, перехоплюємо її і виводимо повідомлення про помилку
